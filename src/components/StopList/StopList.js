  ...
  filterByText(stops) {
    ...
    stops.forEach(stop => {
      if (
        ...
        !(stop.attributes.field_id.indexOf(this.props.filterText) !== -1)
      ) {
        return;
      }
      ...
      <div>
            ...
            <div className={s.listContent}>
              ...
              >
                <h2 className={s.title}>
                  <span>{itineraryStops.itinerary.attributes.name}</span>
                </h2>
              </Sticky>
              <ul>
                {itineraryStops.stops.map(stop => (
                  <li key={stop.id}>
                    <StopTeaser
                      destination={`/stop/${this.props.itinerary_id}/${
                        stop.id
                      }`}
                      stop={stop}
                    />
        ...
        <Modal onClick={e => this.toggle(e)} openModal={this.state.isModalOpen}>
          <div className={s.switchIt}>
            ...
            <button className={s.btn} id="switchItinary">
              <Ionicon
